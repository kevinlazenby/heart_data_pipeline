---
title: "create_pre_policy_dataset"
...
```{r}
cand_thor <- read_sas(here("pubsaf2103", "cand_thor.sas7bdat"), NULL) %>%  
...
stat_just_hr1a <- read_sas(here("pubsaf2103", "statjust_hr1a.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

stat_just_hr1b <- read_sas(here("pubsaf2103", "statjust_hr1b.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

just_form_hr <- read_sas(here("pubsaf2103", "JustFormHR.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

just_form_hr_stat1 <- 
  read_sas(here("pubsaf2103", "JustFormHRStat1.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

just_form_hr_stat2 <- 
  read_sas(here("pubsaf2103", "JustFormHRStat2.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

just_form_hr_stat3 <- 
  read_sas(here("pubsaf2103", "JustFormHRStat3.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

just_form_hr_stat4 <- 
  read_sas(here("pubsaf2103", "JustFormHRStat4.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

risk_strat_data_hr <- 
  read_sas(here("pubsaf2103", "RiskStratDataHR.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

just_form_hr_data_link <- 
  read_sas(here("pubsaf2103", "JustFormHRDataLink.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

column_descriptions <- 
  read_sas(here("pubsaf2103", "ColumnDescriptions.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

table_descriptions <- 
  read_sas(here("pubsaf2103", "TableDescriptions.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

stathist_thor <- 
  read_sas(here("pubsaf2103", "stathist_thor.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()

status_just_episode <- 
  read_sas(here("pubsaf2103", "StatusJustEpisode.sas7bdat"), NULL) %>%
  zap_formats() %>% zap_labels()

thor_support_device <- 
  read_sas(here("pubsaf2103", "ThorSupportDevice.sas7bdat"), NULL) %>% 
  zap_formats() %>% zap_labels()

tx_hr <- read_sas(here("pubsaf2103", "tx_hr.sas7bdat"), NULL) %>%  
  zap_formats() %>% zap_labels()
```
...
# Status 1A justification forms from pre-policy era
statjust_1a_pre <- stat_just_hr1a %>% filter(PX_ID %in% init_list_pre$PX_ID)

# Status 1B justification forms from pre-policy era
...
# 8. Add justification forms for each candidate to init_list_pre 
...
# Reshaping risk stratification data to include rows by unique observation date
