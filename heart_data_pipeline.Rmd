...
#    If logical value above is set appropriately, this chunk only runs if an
#    RData file of raw heart data does not exist (i.e. when the lab receives
#    a new release of the SAF from SRTR)
...
if (RData_file_loaded == FALSE) {
  ...
  raw_data_filename <- "pubsaf2303_raw_heart_data.RData"
...
# 1.5. Load raw data from .RData file

```{r load from RData file}
...
cand_list <- cand_list %>% 
  select(
    ...
    CAN_LISTING_CTR_ID, CAN_REM_DT, CAN_LAST_ACT_STAT_DT, CAN_LAST_INACT_STAT_DT,
...
# last follow-up, status changes, and dates from risk stratification data
