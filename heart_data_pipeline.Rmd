...
just_form <- just_form %>% 
  ...
  mutate(status_description = descrip) %>%
  select(-descrip) %>% 
...
risk_strat <- risk_strat %>% 
  ...
  select(-ChangeUserId, -ChangeRequestorId, -WlregAuditId, -RiskStratId)

combined_just_forms <- just_form %>%
  full_join(risk_strat, by = c("JustId", "PX_ID")) %>% 
...
full_list <- full_list %>% 
  ...
  # fills in missing values for each candidate
  group_by(PX_ID) %>%
  fill(WL_ORG:REC_MM_EQUIV_TX, .direction = "downup") %>%

  # fills in missing values for each candidate, but only fills down to
  # preserve changes in status over time (similar/same mechanism as na.locf)
  arrange(PX_ID, unique_date) %>% 
  group_by(PX_ID) %>%
  fill(CANHX_STAT_CD:ExtInotropeDopamine, .direction = "down") %>%
...
risk_strat <- risk_strat %>%
  ...
  select(PX_ID, RiskStratId, unique_date, unique_event, 
