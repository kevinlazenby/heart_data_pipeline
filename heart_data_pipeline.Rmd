...
just_stat1a_1b <- just_stat1a_1b %>% 
  ...
  mutate(unique_date = case_when(
    ...
    !is.na(unique_date.replaced) ~ 1,
    is.na(unique_date.replaced) ~ 0)) %>% 
...
full_list <- full_list %>%
  ...
  # full join by common variables, thus no need for suffix
  full_join(just_stat1a_1b, by = c("PX_ID", "unique_date",
                                   "unique_event", "CANHX_STAT_CD")) %>%
  ...
  # fill all variables "down" to show change over time
  # this fills in missing values using the na.locf() method
  group_by(PX_ID) %>%
  fill(PX_ID:last_col(offset = 4), .direction = "down") %>%
  ungroup() %>%
...
# 9. Calculating number of days between each observation in full_list, by PX_ID
...
rdata_filename <-  paste0("final_list_", curr_data_release, ".RData")

csv_filename <-  paste0("final_list_", curr_data_release, ".csv")
