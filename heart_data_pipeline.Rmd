...
risk_strat_data_hr <- risk_strat_data_hr %>% 
  ...
  # removes 134 entries that have valid WlregAuditId but missing JustId
  filter(!is.na(JustId)) %>%
  
  # there are 732 times that two just. forms were submitted on the same day for a
...
full_list <- cand_list %>% 
  ...
  group_by(PX_ID, unique_date) %>%
  fill(CANHX_STAT_CD:CAN_PULM_EMBOL, .direction = "downup") %>%
  distinct() %>%
  ...
  group_by(PX_ID) %>%
  fill(WL_ORG:CAN_PULM_EMBOL, .direction = "downup") %>%
...
full_list <- full_list %>% 
  ...
  # removes redundant rows after combining events above
  group_by(PX_ID, unique_date) %>%
  fill(CANHX_STAT_CD:REC_MM_EQUIV_TX, .direction = "downup") %>%
  distinct() %>%

  # fills in missing values for each candidate, which is safe because all vars
  # in cand_list and tx_list only have a single value
  group_by(PX_ID) %>%
...
# 6. Join just_form and just_stat1-just_stat4 with risk_strat, pivot by date, then join with full_list
...
full_list <- full_list %>%
  ...
  # removes redundant rows after combining events above
  group_by(PX_ID, unique_date) %>%
  fill(CANHX_STAT_CD:ExtInotropeDopamine, .direction = "downup") %>%
  ...
  # fill status code "down" to show status changes over time
  group_by(PX_ID) %>% 
  ...
  # fill risk_strat variables "down" to show change over time
  # we are filling missing values using the na.locf() method
  group_by(PX_ID) %>% 
  fill(systolicBP:HrSevFailBnp, .direction = "down") %>%
  ungroup() %>%
...
# 8. Pivot old policy justification forms by date, then join with full_list
...
# 9. Method to filter full_list to exclude unwanted timepoints

```{r filter full_list}

full_list_final <- full_list %>% 
  filter(grepl("CAN_LISTING_DT", unique_event) |
...
full_list_final <- full_list_final %>%
  ungroup() %>% 
  mutate(t_start = 0, t_stop = 0) %>% 
  arrange(PX_ID, unique_date)

for (i in 2:nrow(full_list_final)) {
  if (full_list_final$PX_ID[[i]] == 
      full_list_final$PX_ID[[i-1]]) {
    
    full_list_final$t_stop[[i-1]] = 
      full_list_final$unique_date[[i]] - 
      full_list_final$unique_date[[i-1]] +
      full_list_final$t_start[[i-1]]
    
    full_list_final$t_start[[i]] = full_list_final$t_stop[[i-1]]
    ...
    full_list_final$t_start[[i]] = 0
      
  }
}

full_list_final <- full_list_final %>% 
  select(PX_ID, JustId, unique_event, unique_date, t_start, t_stop, CANHX_STAT_CD:last_col())

filename <-  paste0("full_list_final_", today(), ".RData")

save(full_list_final, file = filename)

```

# Scratch work used to make figure for poster on 6/1/2022

```{r}

var_label(example_list) <- list(HemoSbp = "Systolic blood pressure",
                                      HemoDbp = "Diastolic blood pressure",
                                      HemoPasp = "Pulmonary Artery\nSystolic blood pressure",
                                      HemoMeanPressure = "Mean pulmonary artery pressure",
                                      HemoPadp = "Pulmonary Artery Diastolic BP")

example_list_pivot <- example_list %>% filter(PX_ID == 1248356) %>% select(-HemoCvp, -HemoPcwp) %>% pivot_longer(-(PX_ID:status_description), names_to = "variable", values_to = "pressure", values_drop_na = TRUE)

poster_figure <- ggplot(data = example_list_pivot, aes(x = unique_date, y = pressure)) +
  geom_line(aes(color = variable), size = 1) +
  geom_label(data = example_list_pivot %>% 
               filter(pressure > 88), aes(label = factor(status_description)), nudge_y = 5, alpha = 0.5, size = 8) +
  labs(
    x = "Date of measurement",
    y = "Pressure (mm Hg)",
    color = "Hemodynamic Measurement") +
  scale_color_discrete(breaks = c("HemoSbp", "HemoDbp", "HemoPasp", "HemoMeanPressure", "HemoPadp"),
                       labels = c("Systolic blood pressure", "Diastolic blood pressure",
                                  "Pulmonary artery\nsystolic blood pressure",
                                  "Mean pulmonary artery pressure",
                                  "Pulmonary artery\ndiastolic blood pressure")) +
  scale_x_datetime(date_breaks = "2 days", date_labels = "%B %d", expand = expansion(mult = 0.1)) +
