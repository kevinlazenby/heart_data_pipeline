...
# since this is a left_join, this step excludes 134 PX_IDs that lack a 
# corresponding JustId. 126 of these 134 have CAN_INIT_STAT = 2999
# (inactive status), so I assume these represent data entry errors
...
just_form_hr <- just_form_hr %>% 
  ...
  left_join(status_just_episode %>% select(JustId:EndDate), by = "JustId")
...
# this block selects the 42 variables in cand_thor currently used in analysis
...
# 6. Pivot just_form_hr by date, then join with full_list_post

```{r pivot just_form_hr}

just_form_post <- just_form_post %>% 
  select(PX_ID, RequestedCandStatCd, ExtensionNumber, Exception, 
         ThorSupportDevicePrimary, ThorSupportDeviceSecondary, descrip,
         AdmittedToHospital, DeviceTypeId:EndDate) %>%
  mutate(status_description = descrip, BeginDt = BeginDate, EndDt = EndDate) %>% 
  select(-descrip, -BeginDate, -EndDate) %>% 
  pivot_longer(cols = ends_with("Dt"),
               names_to = "event", 
               values_to = "date", 
               values_drop_na = TRUE) %>%
  ...
  arrange(PX_ID, unique_date) %>% 
  ungroup() %>%
  select(PX_ID, unique_event, unique_date, RequestedCandStatCd, ExtensionNumber,
         Exception, ThorSupportDevicePrimary, ThorSupportDeviceSecondary, 
         status_description, AdmittedToHospital, DeviceTypeId:OtherSpecify)

full_list_post <- full_list_post %>% 
  
  # full join by common variables, thus no need for suffix
  full_join(just_form_post, by = c("PX_ID", "unique_date", "unique_event")) %>% 
  ...
  # removes redundant rows after combining events above
  group_by(PX_ID, unique_date) %>%
  fill(WL_ORG:OtherSpecify, .direction = "downup") %>%
  distinct() %>%
