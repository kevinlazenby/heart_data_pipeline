...
# 3. Pivot cand_thor and tx_hr by date, then full_join
...
full_list_post <- full_list_post %>% 
  full_join(tx_hr_post, by = c("PX_ID", "unique_date"), 
            suffix = c(".cand_thor", ".tx_hr")) %>% 
  arrange(PX_ID, unique_date)
  
```

# 4. Pivot justification forms for each candidate by date
...
full_list_post <- full_list_post %>% 
  full_join(just_stat1_post, by = c("PX_ID", "unique_event", "unique_date")) %>% 
...
# 7. Reshaping risk stratification data to include rows by unique observation date
...
risk_strat_data_hr_wide <- risk_strat_data_hr_long %>%
  ...
  select(PX_ID, RiskStratId, ChangeDate, unique_date, unique_event, t_start, 
         t_stop, CandHistNumSternotomies, CandHistStroke, CandHistThromb,
...
for (i in 2:nrow(risk_strat_data_hr_wide)) {
  if (risk_strat_data_hr_wide$PX_ID[[i]] == 
      risk_strat_data_hr_wide$PX_ID[[i-1]]) {
    
    risk_strat_data_hr_wide$t_stop[[i-1]] = 
      risk_strat_data_hr_wide$unique_date[[i]] - 
      risk_strat_data_hr_wide$unique_date[[i-1]] +
      risk_strat_data_hr_wide$t_start[[i-1]]
    
    risk_strat_data_hr_wide$t_start[[i]] = risk_strat_data_hr_wide$t_stop[[i-1]]
    ...
    risk_strat_data_hr_wide$t_start[[i]] = 0
